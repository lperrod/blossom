...
public class ElasticsearchSearchEngineImpl<DTO extends AbstractDTO> implements
  SearchEngine<QueryBuilder, SearchRequestBuilder, SearchResponse, DTO> {
  ...
  public ElasticsearchSearchEngineImpl(Client client, ObjectMapper objectMapper,
  ...
  private <T> SearchResult<T> doParseResults(SearchResponse searchResponse, Pageable pageable,
    ...
    if (this.aggregationRegistry != null && facetConfigurations != null) {
      ...
      return new SearchResult(
        ...
        new PageImpl<>(resultList, pageable, searchResponse.getHits().getTotalHits() != null ? searchResponse.getHits().getTotalHits().value : 0),
        facets);
    }
