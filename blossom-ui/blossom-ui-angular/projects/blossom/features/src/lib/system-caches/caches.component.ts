...
@Component({
  selector: 'app-caches',
  standalone: true,
  imports: [MatTableModule, MatButtonModule, MatIconModule, MatSlideToggleModule, MatToolbarModule, SearchBarComponent],
...
export class CachesComponent implements OnInit {
  ...
  load(): void {
    this.cachesService.list(this.query).subscribe(data => this.caches.set(data));
  }
