...
export interface GroupDTO { id: string; name: string; description: string; }
...
export class GroupsService {
  ...
  get(id: string): Observable<GroupDTO> { return this.http.get<GroupDTO>(`${this.API}/${id}`); }
  create(form: any): Observable<GroupDTO> { return this.http.post<GroupDTO>(this.API, form); }
  update(id: string, form: any): Observable<GroupDTO> { return this.http.put<GroupDTO>(`${this.API}/${id}`, form); }
