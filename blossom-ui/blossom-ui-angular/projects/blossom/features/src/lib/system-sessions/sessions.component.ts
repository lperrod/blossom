...
@Component({
  ...
  standalone: true,
  imports: [DatePipe, MatTableModule, MatButtonModule, MatIconModule, MatChipsModule],
  ...
  template: `
    ...
    <table mat-table [dataSource]="sessions()" class="full-width">
      ...
      <ng-container matColumnDef="actions">
        ...
        <td mat-cell *matCellDef="let s">
          @if (!s.expired) {
            <button mat-icon-button color="warn" (click)="invalidate(s.sessionId)">
...
export class SessionsComponent implements OnInit {
  ...
  load(): void {
    this.sessionsService.list().subscribe(data => this.sessions.set(data.sessions));
  }
