...
export interface RoleDTO { id: string; name: string; description: string; privileges: string[]; }
...
export class RolesService {
  ...
  get(id: string): Observable<RoleDTO> { return this.http.get<RoleDTO>(`${this.API}/${id}`); }
  create(form: any): Observable<RoleDTO> { return this.http.post<RoleDTO>(this.API, form); }
  update(id: string, form: any): Observable<RoleDTO> { return this.http.put<RoleDTO>(`${this.API}/${id}`, form); }
