...
@Component({
  ...
  imports: [FormsModule, MatCardModule, MatFormFieldModule, MatInputModule, MatButtonModule, MatIconModule, MatTabsModule, PrivilegeTreeComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    @if (role()) {
      <div class="page-header"><h2>{{ role()!.name }}</h2>
        <div>
          @if (editing()) {
...
export class RoleDetailComponent implements OnInit {
  ...
  availablePrivileges = signal<any[]>([]);
  ...
  ngOnInit(): void {
    ...
    this.svc.get(id).subscribe(r => this.role.set(r));
    this.svc.getPrivileges().subscribe(p => this.availablePrivileges.set(p));
  }
  onPrivilegesChange(privs: string[]): void { const r = this.role(); if (r) { r.privileges = privs; } }
  save(): void { if (!this.role()) return; this.svc.update(this.role()!.id, this.role()!).subscribe({ next: r => { this.role.set(r); this.editing.set(false); this.notify.success('Role updated'); }, error: () => this.notify.error('Failed') }); }
